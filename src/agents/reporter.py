...
class ReporterAgent:
    ...
    def run(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            state.status = "done"
            state.confidence["reporting"] = self._calculate_confidence(
                has_pr=bool(state.pr_url),
                test_success=state.test_results.get("success", False) if state.test_results else False,
            )
            
            logger.info(f"Reporter: completed - PR: {state.pr_url} (confidence: {state.confidence.get('overall', 0):.2f})")
...
"""
    ...
    def _build_jira_comment(self, state: AgentState) -> str:
        ...
        confidence_section = self._format_confidence(state.confidence)
...
"""
    ...
    def _format_confidence(self, confidence: dict) -> str:
        """Format confidence scores for Jira comment."""
        lines = []
    ...
    def _calculate_confidence(self, has_pr: bool, test_success: bool) -> float:
        """Calculate reporting confidence."""
        score = 0.5
    ...
    def _send_discord_notification(self, state: AgentState) -> None:
        ...
        overall_conf = state.confidence.get("overall", 0) * 100
        
        details = f"""**Task**: {state.jira_ticket_id} - {summary}
**Status**: ✅ Completed → In Review
**Confidence**: {overall_conf:.0f}%
**Pull Request**: {state.pr_url or 'Pending'}
